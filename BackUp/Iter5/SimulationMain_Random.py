...
N = 20
NumberOfCars = 1
MaxNumberOfNodesPerCar = int(N/(NumberOfCars-1)) if NumberOfCars > 1 else N
# MaxNumberOfNodesPerCar = 1
NumberOfChargeStations =  int(np.ceil(N/15))
...
NominalPlan.ChargingStations = np.array([19,7,18]).reshape((-1,1))
...
BestPlan = DataTypes.BestPlan(N, NominalPlan.ChargingStations)
t = time.time()
...
plt.xlabel('Nodes')
FileName = "OptGurobi"+"_N"+str(N)+"_Cars"+str(NumberOfCars)+"_MaxNodesPerCar"+str(MaxNumberOfNodesPerCar)+"_MaxNodesToSolver"+"_RechargeModel"+str(PltParams.RechargeModel)+'.png'
plt.savefig("Results//Nodes_"+FileName)
############################################################################################################
...
for m in range(NumberOfCars):
    ...
    print('Car '+str(m+1)+' '+str(SolutionProbabilityTimeReliability*100)+'% Lap Time: '+"{:.2f}".format(sorted[indx])+', '+str(int(95))+'% Lap Time: '+"{:.2f}".format(sorted[int(0.95*Nmc)]))
    print('Car '+str(m+1)+' Has a '+"{:.2f}".format(100*np.sum(MinEnergyLevel[:,m]<0)/Nmc)+'% chance of running out of energy')
    leg_str1.append('Car '+' '+str(SolutionProbabilityTimeReliability*100)+'% Lap Time: '+"{:.2f}".format(sorted[indx])+', '+str(int(95))+'% Lap Time: '+"{:.2f}".format(sorted[int(0.95*Nmc)]))
...
plt.legend(leg_str2)
plt.savefig("Results//MC_"+FileName)
plt.show()
