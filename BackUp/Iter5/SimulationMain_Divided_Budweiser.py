...
Scenario = -1 # -1: Mean and Std form data, 0-23: Specific Scenario
CostFunctionType = 1 # 1: Min Sum of Time Travelled, 2: ,Min Max Time Travelled by any car
MaxTotalTimePerVehicle = 150.0
MaxNumberOfNodesPerCar = 13
SolverType = 'Recursive' # 'Gurobi' or 'Recursive'
...
if Scenario >= 0:
    ...
    NominalPlan.NodesEnergyTravel = BudweiserMeanData['ec'] * EnergyFactor
    NominalPlan.NodesEnergyTravelSigma = BudweiserStdData['ec_based_on_soc'] * EnergyFactor
    NominalPlan.NodesEnergyTravelBasedOnSOC = BudweiserMeanData['ec_based_on_soc'] * EnergyFactor
...
for i in range(NominalPlan.N):
    NominalPlan.NodesTimeOfTravel[i,i] = 999.0
    NominalPlan.TravelSigma[i,i] = 999.0
    NominalPlan.TravelSigma2[i,i] = 999.0
    NominalPlan.NodesEnergyTravel[i,i] = -999.0
    NominalPlan.NodesEnergyTravelSigma[i,i] = -999.0
    NominalPlan.NodesEnergyTravelSigma2[i,i] = 999.0
...
t = time.time()
NodesTrajectory = np.zeros((N,NumberOfCars), dtype=int)
EnergyEnteringNodes = np.zeros((N,))
ChargingTime = np.zeros((N,))
EnergyExitingNodes = np.zeros((N,))
Cost = 0.0
for Ncar in range(NumberOfCars):
    n = len(NodesGroups[Ncar])
    NominalPlanGroup = CreateSubPlanFromPlan(NominalPlan, NodesGroups[Ncar])

    if SolverType == 'Gurobi':
        NodesTrajectoryGroup, CostGroup, EnergyEnteringNodesGroup, ChargingTimeGroup, EnergyExitingNodesGroup = SolveGurobi_Convex_MinMax(PltParams=PltParams,
                                                                NominalPlan= NominalPlanGroup,
                                                                TimeLeft= MaxMissionTime,
                                                                PowerLeft= InitialChargeStage,)
    else:
        BestPlan, NodesTrajectoryGroup, CostGroup, ChargingStationsDataGroup = SolveParallelRecursive_ChargingStations(PltParams=PltParams,
                                                                NominalPlan= NominalPlanGroup,
                                                                i_CurrentNode = 0, 
                                                                TourTime = 0.0,
                                                                TourTimeUncertainty = 0.0,
                                                                EnergyLeft = InitialChargeStage,
                                                                EnergyLeftUncertainty = 0.0,
                                                                ChargingStationsData = DataTypes.ChargingStations(NominalPlanGroup.ChargingStations),
                                                                NodesTrajectory = [], 
                                                                BestPlan = DataTypes.BestPlan(NominalPlanGroup.N, NominalPlanGroup.ChargingStations, time.time()))
        NodesTrajectoryGroup = np.array(NodesTrajectoryGroup).reshape((-1,1))
    ChargingTime = np.zeros((N,))
    Cost += CostGroup
    for i in range(len(NodesTrajectoryGroup)):
        NodesTrajectory[i,Ncar] = NodesGroups[Ncar][NodesTrajectoryGroup[i]][0]
...
print(SolverType+' Trajectory Time = ', np.sum(np.max(TimeVec+NominalPlan.EnergyAlpha*EnergySigma, axis=0)))
