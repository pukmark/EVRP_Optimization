...
PltParams.BatteryCapacity = 100
PltParams.MinimalSOC = 0.1*PltParams.BatteryCapacity
...
## Charging Stations:
NominalPlan.ChargingStations = np.floor(np.random.uniform(1,N,size=(NumberOfChargeStations,1)))
...
for SolverType in SolverTypes:
    ...
    if SolverType == 'CVX':
        t = time.time()
        NodesTrajectory, Cost, PowerLeft, ChargingTime = SolveCVX_ChargingStations_MinMax(PltParams=PltParams,
