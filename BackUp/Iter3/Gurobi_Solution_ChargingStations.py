...
def SolveGurobi_ChargingStations_MinMax(PltParams, NominalPlan, NodesWorkDone, TimeLeft, PowerLeft, i_CurrentNode, NodesTrajectory, NodesWorkSequence, Cost):
    ...
    for i in range(0, n):
        if np.any(i == NominalPlan.ChargingStations):
            ...
            model.addConstr(a2y[i]==0.0)
            model.addConstr(expy[i]==1.0)
        ...
        for j in range(1, n):
            if i != j:
                ...
                if PltParams.RechargeModel == 'ConstantRate':
                    model.addConstr( eSigma2[j] + (1-DecisionVar[i,j])*PltParams.BatteryCapacity >= eSigma2[i] + DecisionVar[i,j]*NominalPlan.NodesEnergyTravelSigma[i,j]**2 )
                    model.addConstr( e[j] + (DecisionVar[i,j]-1)*PltParams.BatteryCapacity <= e[i] - 0*EnergyAlpha*eSigma[j] + NominalPlan.NodesEnergyTravel[i,j]*DecisionVar[i,j] + y[i]*NominalPlan.StationRechargePower ) # Energy Sumation over edges
                else:
                    model.addConstr( e[j] + (DecisionVar[i,j]-1)*PltParams.BatteryCapacity <= NominalPlan.NodesEnergyTravel[i,j]*DecisionVar[i,j] + (a1 + (e[i]-a1)*expy[i]) ) # Energy is always positive
    
    # Add the energy constraints for the return to base case
    if NominalPlan.ReturnToBase == True:
        for i in range(1, n):
            if PltParams.RechargeModel == 'ConstantRate':
                model.addConstr(PltParams.MinimalSOC + (DecisionVar[i,0]-1)*PltParams.BatteryCapacity <= e[i] - EnergyAlpha*eSigmaF[i] + NominalPlan.NodesEnergyTravel[i,0] + y[i]*NominalPlan.StationRechargePower )
            else:
                model.addConstr(PltParams.MinimalSOC + (DecisionVar[i,0]-1)*PltParams.BatteryCapacity <= NominalPlan.NodesEnergyTravel[i,0] + (a1 + (e[i]-a1)*expy[i]) )
    ...
    # model.addConstrs((z >= Tf[i]+alpha*SigmaFactor*(1+0.5*(SigmaTf2[i]-SigmaFactor**2)/SigmaFactor**2) for i in range(n)), name="max_contraint")
    for i in range(0, n):
    ...
    return NodesTrajectory, model.ObjVal, e.X, y.X
