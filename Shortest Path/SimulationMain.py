...
U, S, Vh = np.linalg.svd(NominalPlan.NodesTimeOfTravel, full_matrices=True)
TimeEst = np.zeros((N,N))
for i in range(N):
    if S[i] < np.sqrt(S[0]) or i>N/10: break
    TimeEst += S[i]*np.outer(U[:,i],Vh[i,:])
print('Time Estimation Error[%]: ', 100*np.linalg.norm(TimeEst-NominalPlan.NodesTimeOfTravel)/np.linalg.norm(NominalPlan.NodesTimeOfTravel))
print('Norm Diff=', np.linalg.norm(TimeEst-NominalPlan.NodesTimeOfTravel))
print('Norm=', np.linalg.norm(NominalPlan.NodesTimeOfTravel))

# plt.figure()
# plt.imshow(NominalPlan.NodesTimeOfTravel)
# plt.colorbar()
# plt.title('Nominal Time of Travel')
# plt.figure()
# plt.imshow(TimeEst)
# plt.colorbar()
# plt.title('Est Time of Travel')
# plt.show()


# NominalPlan.NodesTimeOfTravel = TimeEst
